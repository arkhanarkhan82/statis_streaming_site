...
league_map = load_json(LEAGUE_MAP_PATH) # READ ONLY for Engine
image_map = load_json(IMAGE_MAP_PATH)   # Read & Write for Engine (Logos)
...
def smart_resolve(raw_match):
    # --- FIX APPLIED HERE: Handle NoneType ---
...
def main():
    ...
    for m in raw_matches:
        ...
        # Resolve Images (Updates Image Map ONLY)
    ...
    # === SAFETY CHECK: SAVE IMAGE MAP ONLY ===
    # We do NOT save league_map.json. It is managed by Admin Panel only.
    ...
    for key, settings in PRIORITY_SETTINGS.items():
        ...
        pg = l_tpl_base.replace('{{PAGE_FILTER}}', key)
